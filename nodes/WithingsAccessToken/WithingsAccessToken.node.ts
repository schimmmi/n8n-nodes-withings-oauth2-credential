...
export class WithingsAccessToken implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ["transform"],
		version: 1,
		description: "Get Withings access token from authorization code",
		defaults: {
		...
		properties: [
			...
			},
			{
				...
				required: true,
				default: "",
				description: "The authorization code from Withings OAuth2 callback URL",
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				// Withings token exchange with required action=requesttoken parameter
